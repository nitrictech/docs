export const description = 'Comparison to Ampt'
...
3. **Infrastructure Provisioning:** Nitric uses Pulumi by default for provisioning cloud resources, but also allows the use of [custom providers](/reference/providers/custom/building-custom-provider). Ampt uses a hosted provisioning engine via their online platform. While resources deployed with Nitric are statically determined at deployment, Ampt is able to reallocate resources between AWS services at runtime to optimize applications.
...
| Extensibility        | [Custom providers](/reference/providers/custom/building-custom-provider) can be created      | None                                                                 |
...
| Cloud providers      | AWS, Azure, GCP and [Custom providers](/reference/providers/custom/building-custom-provider) | Ampt Platform (AWS)                                                  |
