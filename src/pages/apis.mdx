...
func main() {
	...
	secureApi, err := nitric.NewApi(
		"main",
		nitric.WithSecurity(defaultSecurityRule([]string{"user.read"})),
	)
	if err != nil {
		return
	}
...
```go {{ tag: 'resources/main.go' }}
package resources

import "github.com/nitrictech/go-sdk/nitric"

var AccountsApi nitric.Api
...
```dart {{ tag: 'common/resources.dart' }}
import 'package:nitric_sdk/nitric.dart';
