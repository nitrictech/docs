...
Topics and Events provide a scalable, decoupled, way to communicate between services.

### Topics

A topic is a named target where events can be published. They can be thought of as a subject that your services can discuss with each other.

They're awesome for allowing serverless services to communicate in a stateless, scalable and highly decoupled way.
...
A subscription is something that listens to a topic. You can think of it as a channel that notifies your services when something new arrives on the topic.
...
To send an event to a topic and notify all subscribers, use the `publish()` method on the topic reference. The service must have permissions to `publish` to the topic.
...
The limitation exists to protect you from infinite loops in deployed services where a service calls itself indirectly via a topic. These sorts of mistakes can lead to large unintentional cloud charges - something you probably want to avoid.
...
Queues and Tasks provide a scalable, decoupled, way for services to communicate asynchronously.

### Queues

Queues are named targets where tasks can be sent. They can be thought of as a group of related tasks. Unlike [topics](#topics), tasks sent to a queue won't automatically trigger services to process them. Instead, services receive tasks from the queue by requesting them.
...
Nitric allows you to define named queues. When defining queues, you can give the service permissions for receiving and sending.
...
When you pull tasks from a queue, they are not immediately deleted. Instead, they are leased, which means they are temporarily hidden from other services until the lease expires.
...
The below example receives 10 tasks using the `receive()` method, and then acknowledges the task as completed using the `complete()` method. You'll note that the service has permissions to receive messages.
