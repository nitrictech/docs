...
var userCreatedTopic = Nitric.Topic<User>("user-created").With(TopicPermission.Publishing);
...
public class Application {
	public static void main(String[] args) {
		var userCreatedTopic = Nitric.INSTANCE.topic("user-created").with(TopicPermission.Publishing);
...
fun main() {
	val userCreatedTopic = Nitric.topic("user-created").with(TopicPermission.Publishing);
...
var updates = Nitric.Topic<User>("updates").With(TopicPermission.Publishing);

updates.Publish(new User { Name = "John Smith", Email = "john.smith@email.com" });
...
public class Application {
	public static void main(String[] args) {
		...
		// Specifying a map will produce a Nitric event with a generated ID and 'none' payloadType
		userCreatedTopic.publish(Map.of(
			"email", "new.user@example.com"
		));

		// You can also specify a Nitric event with a set ID and payload type
		userCreatedTopic.publish(new NitricEvent(
			Map.of("email", "new.user@example.com"), // payload
			"0F95D606-61ED-4B6E-B82C-80490948F7D7", // ID
			"none" // payload type
		));
...
fun main() {
	...
	// Both of these will publish the same event
	userCreatedTopic.publish(mapOf(
		"email" to "new.user@example.com"
	))

	userCreatedTopic.publish(NitricEvent(
		payload = mapOf(
			"email" to "new.user@example.com"
		)
	))
...
var updates = Nitric.Topic<User>("updates");

updates.Subscribe(ctx =>
{
  var profile = ctx.Req.Payload();
...
public class Application {
	public static void main(String[] args) {
		var userCreatedTopic = Nitric.INSTANCE.topic("user-created");

		userCreatedTopic.subscribe((ctx) -> {
			var request = ctx.getReq().json(NewUserEvent.class);
...
data class NewUserEvent(val email: String)

fun main() {
	val userCreatedTopic = Nitric.topic("user-created")

	userCreatedTopic.subscribe { ctx ->
		val request = ctx.req.json<NewUserEvent>()
		sendWelcomeEmail(request)
		ctx
	}
...
func main() {
  loopTopic, err := nitric.NewTopic("updates").With(nitric.TopicPublishing)
  if err != nil {
    return
  }
...
var loopTopic = Nitric.Topic<User>("updates").With(TopicPermission.Publishing);

updates.Subscribe(ctx =>
{
  loopTopic.Publish(new User { Name = "John Smith", Email = "john.smith@email.com" });
...
public class Application {
	public static void main(String[] args) {
		var loopTopic = Nitric.INSTANCE.topic("updates").with(TopicPermission.Publishing);

		loopTopic.subscribe((ctx) -> {
			loopTopic.publish(Map.of())
			return ctx;
		})
...
fun main() {
	val loopTopic = Nitric.topic("updates").with(TopicPermission.Publishing)

	loopTopic.subscribe { ctx ->
		loopTopic.publish(mapOf())
		ctx
	}
...
public class Application {
	public static void main(String[] args) {
		var updates = Nitric.INSTANCE.topic("updates");

		updates.subscribe((ctx) -> {
			if (isDuplicate(ctx.getReq())) {
				return ctx;
			}
			// Not a duplicate, process the event
			// ...
...
fun main() {
	val loopTopic = Nitric.topic("updates")
...
func main() {
  transactionQueue, err := nitric.NewQueue("transactions").With(nitric.QueueSending, nitric.QueueReceiving)
  if err != nil {
    fmt.Prinln(err)
    return
  }
...
var transactionQueue = Nitric.Queue<TransactionRequest>("transactions").With(QueuePermission.Sending, QueuePermission.Receiving);
...
public class Application {
	public static void main(String[] args) {
		var transactionQueue = Nitric.INSTANCE.queue("transactions")
			.with(QueuePermission.Send, QueuePermission.Receive);
...
fun main() {
	val transactionQueue = Nitric.queue("transactions")
		.with(QueuePermission.Send, QueuePermission.Receive)
...
func main() {
  transactionQueue, err := nitric.NewQueue("transactions").With(nitric.QueueSending)
  if err != nil {
    fmt.Prinln(err)
    return
  }
...
transactionQueue.Send(new TransactionRequest { Message = "hello world" });
...
public class Application {
	public static void main(String[] args) {
		var transactionQueue = Nitric.INSTANCE.queue("transactions")
			.with(QueuePermission.Send);

		transactionQueue.send(Map.of("message", "hello world"));
...
fun main() {
	...
	transationQueue.send(mapOf("message" to "hello world"))
...
transactionQueue.Send(
  new TransactionRequest { Message = "batch task 1" },
  new TransactionRequest { Message = "batch task 2" }
);
...
public class Application {
	public static void main(String[] args) {
		...
		transactionQueue.send(
			new Task(Map.of("message", "batch task 1"), "task-1", "1", "none"),
			new Task(Map.of("message", "batch task 2"), "task-2", "2", "none")
		);
...
fun main() {
	...
	transationQueue.send(
		Task(payload = mapOf("message" to "batch task 1")),
		Task(payload = mapOf("message" to "batch task 2"))
	)
...
func main() {
  transactionQueue, err := nitric.NewQueue("transactions").With(nitric.QueueReceiving)
  if err != nil {
    return
  }
  ...
  tasks, err := transactions.Receive(ctx, 10)
  if err != nil {
    return
  }
...
var tasks = transactionQueue.Receive();

tasks.ForEach(task => {
  Console.WriteLine(task.Payload.Message);
...
public class Application {
	public static void main(String[] args) {
		var transactionQueue = Nitric.INSTANCE.queue("transactions")
			.with(QueuePermission.Receive);

		// Receive up to 10 tasks from the queue
		var tasks = transactionQueue.receive(10);
...
fun main() {
	...
	// Receive up to 10 tasks from the queue
	val tasks = transactionQueue.receive(10)
