...
Nitric supports monorepos via the custom runtime feature, this allows you to change the build context of your docker build. To use a custom runtime in a monorepo, you can specify the runtime key per service as shown below.

<Note>Available in Nitric CLI version 1.45.0 and above</Note>

### Example for Turborepo

[Turborepo](https://turbo.build/) is a monorepo tool that allows you to manage multiple packages in a single repository. In this example, we will use a custom runtime to build a service in a monorepo using a custom dockerfile.
