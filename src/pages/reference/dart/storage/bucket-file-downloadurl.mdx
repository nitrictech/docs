export const description =
  "Reference for Nitric's Dart library - Get a download url for a file from a bucket."
...
final images = Nitric.bucket("assets").requires([
  BucketPermission.reading,
]);
...
mainApi.get("/images/:id", (ctx) async {
  ...
  ctx.resp.status = 303;
  ctx.resp.headers["Location"] = [signedUrl];
