...
Unlike the other websocket events the `connect` event has access to the query parameteres provided during the connection request. This allows the client to provide relevant information needed to validate and establish the connection.
...
Websocket connections need to be managed. This can be done using any persistent datastore you like. One approach is to use a [Nitric collection](https://nitric.io/docs/collections).
