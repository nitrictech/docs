export const description =
  "Reference for Nitric's Node.js library - Send a message to a connected websocket client"
...
const sendMessage = async (connectionId, message) => {
  const env = new TextEncoder();
  await socket.send(connectionId, enc.encode(message))
}
```
...
<Properties>
  ...
  <Property name="message" required type="string | Uint8Array | Record<string, any>">
