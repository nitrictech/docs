...
The Nitric CLI would take these code declarations and send it to the provider for deployment. The provider will then convert this declaration to actual cloud infrastructure. The specific cloud service that the provider will use for each Nitric resource is listed in the [table below](#standard-providers).
