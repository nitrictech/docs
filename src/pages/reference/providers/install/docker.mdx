export const description = "Nitric's Providers distributed via containers."
...
Nitric Providers run as Docker containers is current a Preview Feature. To enable this feature, add/update the preview feature flag in your `nitric.yaml` file:
...
```yaml {{ tag: "nitric.test.yaml" }}
provider: docker://my-provider
```

<Note>
The `docker://` prefix is required to indicate that the provider is a Docker container image.
</Note>
...
Tags are also suppported and recommended to ensure the correct version of the provider is used. For example, to use a custom provider in a container image named `my-provider` with the tag `latest`, update the stack file as follows :
