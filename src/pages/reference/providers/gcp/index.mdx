...
The GCP provider is supported by the Nitric SDKs and CLI by default. However, credentials for a GCP account will be required when using the [up command](/reference/cli) from the CLI.

## Google CLI Installation

Installing the GCP CLI assists with credentials setup. You can install it using these summarized instructions, for more options see the [Google docs](https://cloud.google.com/sdk/install).
