export const description =
  "Reference for Nitric's .NET library - Sets the cron expression and one or many handlers to be triggered."
...
```csharp
Nitric.Sdk.Nitric.Schedule("send-reminder").Cron("0 1 1 * *", ctx => {
    // do some processing
...
// every 15 minutes
Nitric.Sdk.Nitric.Schedule("check for updates").Cron("0/15 * * * *", ctx => {
    Console.WriteLine("checking for updates");
});

// at 1:00 am on the 1st of every month
Nitric.Sdk.Nitric.Schedules("delete stale data").Cron("0 1 1 * *", ctx => {
