export const description =
  "Reference for Nitric's Go library - Register a handler for HTTP PUT requests to the route."
...
func main() {
	...
	api.NewRoute("/hello").Put(func(ctx *faas.HttpContext, next faas.HttpHandler) (*faas.HttpContext, error) {
...
```go
api.NewRoute("/hello/:name").Put(func(ctx *faas.HttpContext, next faas.HttpHandler) (*faas.HttpContext, error) {
...
func main() {
  ...
  api.NewRoute("/public").Put(func(ctx *faas.HttpContext, next faas.HttpHandler) (*faas.HttpContext, error) {
    // Handle request
    return next(ctx)
...
func main() {
  ...
  api.NewRoute("/hello/:name").Put(faas.ComposeHttpMiddleware(authMiddleware, handleRequest))
...
The PUT request body is accessible using `ctx.Request.Data()`.
