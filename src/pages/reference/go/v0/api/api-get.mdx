export const description =
  "Reference for Nitric's Go library - Register an API route and set a specific HTTP GET handler on that route."
...
<Note>
  ...
  [NewApi().NewRoute().Get()](./api-route-get)
</Note>
...
func main() {
	...
	api.Get("/hello", func(ctx *faas.HttpContext, next faas.HttpHandler) (*faas.HttpContext, error) {
...
```go
api.Get("/hello/:name", func(ctx *faas.HttpContext, next faas.HttpHandler) (*faas.HttpContext, error) {
...
func main() {
  ...
  api.Get("/public", func(ctx *faas.HttpContext, next faas.HttpHandler) (*faas.HttpContext, error) {
    // Handle request
    return next(ctx)
...
func main() {
  ...
  api.Get("/hello/:name", faas.ComposeHttpMiddleware(authMiddleware, handleRequest))
