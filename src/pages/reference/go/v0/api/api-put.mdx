export const description =
  "Reference for Nitric's Go library - Register an API route and set a specific HTTP PUT handler on that route."
...
func main() {
	...
	api.Put("/hello", func(ctx *faas.HttpContext, next faas.HttpHandler) (*faas.HttpContext, error) {
...
```go
api.Put("/hello/:name", func(ctx *faas.HttpContext, next faas.HttpHandler) (*faas.HttpContext, error) {
...
func main() {
  ...
  api.Put("/public", func(ctx *faas.HttpContext, next faas.HttpHandler) (*faas.HttpContext, error) {
    // Handle request
    return next(ctx)
...
func main() {
  ...
  api.Put("/hello/:name", faas.ComposeHttpMiddleware(authMiddleware, handleRequest))
