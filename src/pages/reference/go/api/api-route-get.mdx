...
```go
api.NewRoute("/hello/:name").Get(func(ctx *apis.Ctx) {
...
func main() {
  ...
  api.NewRoute("/public").Get(func(ctx *apis.Ctx) {
    // Handle request
...
func main() {
  ...
  api.NewRoute("/hello/:name").Get(nitric.Compose(authMiddleware, handleRequest))
