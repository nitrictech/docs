export const description =
  "Reference for Nitric's v0 Go library - Register a handler for HTTP POST requests to the route."
...
func main() {
  ...
  api.NewRoute("/hello").Post(func(ctx *handler.HttpContext, next handler.HttpHandler) (*handler.HttpContext, error) {
...
```go
api.NewRoute("/hello/:name").Post(func(ctx *handler.HttpContext, next handler.HttpHandler) (*handler.HttpContext, error) {
...
func main() {
  ...
  api.NewRoute("/public").Post(func(ctx *handler.HttpContext, next handler.HttpHandler) (*handler.HttpContext, error) {
    // Handle request
    return next(ctx)
...
func main() {
  ...
  api.NewRoute("/hello/:name").Post(handler.ComposeHttpMiddleware(authMiddleware, handleRequest))
