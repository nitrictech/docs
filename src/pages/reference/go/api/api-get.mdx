...
func main() {
  ...
  api.Get("/hello", func(ctx *handler.HttpContext, next handler.HttpHandler) (*handler.HttpContext, error) {
...
func main() {
	...
	secureApi.Get("/public", func(ctx *handler.HttpContext, next handler.HttpHandler) (*handler.HttpContext, error) {
		// Handle request
		return next(ctx)
...
func main() {
  ...
  api.Get("/hello/:name", handler.ComposeHttpMiddleware(authMiddleware, handleRequest))
