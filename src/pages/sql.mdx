...
<Note>
  ...
  AWS using the `nitric/aws` provider.
</Note>
...
<Note>Nitric doesn't currently support rolling back migrations, files ending in `.down.sql` are ignored.</Note>
...
<Note>
  ...
  in a Docker container, which will be built and pushed to a registry by your chosen provider.
</Note>
