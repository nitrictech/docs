...
func main() {
  countries, err := nitric.NewCollection("Countries").With(nitric.CollectionReading, nitric.CollectionWriting, nitric.CollectionDeleting)
  if err != nil {
    return
  }
...
var countries = Nitric.Collection<Country>("Countries").With(
...
func main() {
  countries, err := nitric.NewCollection("Countries").With(nitric.CollectionWriting)
  if err != nil {
    return
  }
...
var countries = Nitric.Collection<Country>("Countries").With(CollectionPermission.Writing);
...
func main() {
  countries, err := nitric.NewCollection("Countries").With(nitric.CollectionReading)
  if err != nil {
    return
  }
...
var countries = Nitric.Collection("Countries").With(CollectionPermission.Reading);
...
func main() {
  ...
  countryApi.Get("/country/:name", func(ctx *faas.HttpContext, next faas.HttpHandler) (*faas.HttpContext, error) {
    ...
    country, err := countries.Doc(name).Get(ctx.Request.Context())
    if err != nil {
      return ctx, err
    }
...
var countryApi = Nitric.Api("Countries");

var countries = Nitric.Collection<Country>("Countries").With(CollectionPermission.Reading);

countryApi.Get("/country/:name", (ctx) => {
...
// Fetch next page
if (pagingToken != null) {
  var results = query.PagingFrom(pagingToken).Fetch();
...
```csharp
// Streaming is currently not supported in C#
```
...
// Paging
var results = query.PagingFrom(results.PagingToken).Fetch();
```
