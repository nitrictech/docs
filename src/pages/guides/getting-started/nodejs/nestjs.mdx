...
We then want to add the `http` type to our main function, and pass in the Nest.js application.

<Note>
  The `http` wrapper looks for a `listen` function on the app parameter, which
  is passed the port and callback when the application is started.
</Note>
...
Proxy | Endpoint
3000  | http://localhost:4001

Dev Dashboard | http://localhost:49152
```

You can use any http client to test the application. It should be running on port 3000, but accessible from port 4001 as well. It is important that http://localhost:4001 correctly wraps your application.
