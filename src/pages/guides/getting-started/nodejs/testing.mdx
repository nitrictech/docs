...
```javascript {{ tag: "resources/apis.js" }}
import { api } from '@nitric/sdk'

export const helloApi = api('main')
```

```javascript {{ tag: "resources/buckets.js" }}
import { bucket } from '@nitric/sdk'

export const imageBucket = bucket('images')
```

We want to be able to test individual API route handler callbacks without running the entire API or application. Since testing anonymous functions in the route callback can be difficult, we'll separate the callbacks into their own files to help with isolation.

```javascript {{ tag: "handlers/hello.js" }}
...
Next create a `test` directory, then add the example test file as shown below. The logic being split into separate functions makes it very easy to test our API.
...
```javascript {{ tag: "test/example.test.js" }}
...
There are many ways to perform integration tests in Nitric projects. In this example we'll use a testing framework like [jest](https://jestjs.io/), and use an HTTP testing library like [supertest](https://github.com/visionmedia/supertest) to test an HTTP APT.
...
Now we can start writing test. First create a `test` test directory, then add a test file to it named `integration.test.ts`. For the test, we want to create an agent using supertest, then point the agent at the URL of our API.

```javascript {{ tag: "tests/integration.test.js" }}
...
```javascript {{ tag: "tests/integration.test.ts" }}
import supertest from 'supertest';
import assert from 'assert';
import { imageBucket } from '../resources/buckets';
...
We can then start the API with `nitric start` and run your function code. Finally, in a separate terminal run the tests:
...
The first step is to ensure the application is deployed before running our tests. The [quickstart](/guides/getting-started/quickstart#deploying-the-app) provides details on how to do this for the first time if you're unfamiliar.
