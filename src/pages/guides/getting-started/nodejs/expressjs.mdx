...
<Note>
This guide relies on the HTTP feature of Nitric which is currently in preview. See the docs for this feature to [see how to enable it](/http).
</Note>
...
For this example let's replace the functions folder with a single entrypoint file called `app.js`.
...
At this point we can stop the running application and try deploying to a cloud provider.

## Deploy to the cloud

This is where the value of Nitric really shows through. You don't need to perform any manual cloud deployment activities or add solutions like Terraform to get this project into your cloud environment, Nitric takes care of that for you.
...
The new stack command can help you create the stack by following this prompts, in this case we'll use AWS as an example:
...
Go ahead at test your new Express+Nitric app in the cloud, you can start with the API Gateway URL returned by the `up` command.
