export const description = 'Secure a Nitric API with Amazon Cognito'
...
const helloApi = api('main', {
  ...
  securityDefinitions: {
    cognito: {
      kind: 'jwt',
      issuer: 'https://cognito-idp.<region>.amazonaws.com/<user-pool-id>/.well-known/openid-configuration',
...
<Note>
  ...
  testing nitric applications locally. Currently, they're only enforced when
...
type AuthContext = faas.HttpContext & { user: AccessToken }
 ...
 */
const authorizeAuthors: faas.HttpMiddleware = (ctx: AuthContext, next) => {
