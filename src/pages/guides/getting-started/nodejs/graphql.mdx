...
const profiles = kv('profiles').for('getting', 'setting')
...
We can then use the kv within these services. Each resolver will receive an `args` object which holds the graphql arguments from the query.
