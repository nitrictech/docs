...
```prisma {{ label: "prisma/schema.prisma" }}
...
![create function hook screenshot - step 2](/images/guides/planetscale/connect-with-prisma.png)
...
```typescript {{ label: "resources/apis.ts" }}
...
```typescript {{ label: "resources/buckets.ts" }}
...
```typescript {{ label: "functions/templates.ts" }}
...
```typescript {{ label: "functions/memes.ts" }}
import Jimp from 'jimp'
