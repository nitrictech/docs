...
deploy:
  ...
  before_script:
    ...
    - docker buildx create --driver=docker-container --name=buildkit-builder --driver-opt network=host --use
    - apk add nodejs npm
...
- DOCKER_HOST
  - Set the docker host to `tcp://docker:2375` so Docker will communicate with the docker-in-docker daemon started inside of the service.
...
- OSTYPE
  ...
  - The operating system which the Nitric CLI is installing on. Gitlab CI clears the OSTYPE by default, meaning this value is normally empty.
...
We then need to download the `docker buildx` plugin for Docker. `buildx` is required by the Nitric CLI as it builds the cloud-compatible images before deploying.
...
Then install additional project-specific dependencies like `nodejs` and `npm`.

```yaml
before_script:
  - ...
  - apk add nodejs npm
  - apk add --no-cache --virtual .gyp python3 make g++ # Dependencies of `node-gyp`
```

Finally, add `yarn` using `npm`.
...
<Note>
  `nodejs`, `yarn` and `npm` are only required for Node.js projects. A Python
  project might require `pip` and `pipenv` instead.
</Note>
