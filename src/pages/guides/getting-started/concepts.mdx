...
Even though your apps are portable, they'll still make great use of fully-managed and serverless offerings from each cloud provider. Nitric deploys using services like Lambda, CloudRun, DynamoDB, FireStore, CosmosDB, SNS, Event Grid, PubSub... [the list](/reference/providers) is super long. Nitric also makes sure IAM and other access are correctly configured in your deployed applications, so everything stays secure and just works.
...
If you want to learn more about how Nitric's services are implemented on individual clouds, you can read our [reference](/reference/providers) docs for details.
