export const description =
  'Use the Nitric framework in python to generate and share a daily report using Google Sheets and Google Drive.'
...
Create a project directory, initialize a Nitric project, and install necessary Python packages.
...
## Create, Populate and Share Google Sheet
...
Create a file named `report.py`, where you'll implement your scheduled task service.

> Note: Nitric allows you to set schedules using simple expressions e.g. `7 days`, you can also use [cron expressions](/schedules#using-cron) for more complicated requirements.
...
We can now update our environment variables, create a file named `.env` and set the following values.
...
Start the Nitric server to emulate cloud services:
...
Without creating a separate IaC project, we can immediately deploy our application to the cloud. To do this start by setting up our credentials and any configuration for the cloud you prefer:
