export const description = 'How to work with Nitric SQL Databases and Drizzle'
...
Ensure your CLI is at least `v1.42` and your AWS provider is at least `v1.6.1`.
...
Note that we are also defining a custom migrations dockerfile, this will be used during deployments to apply migrations.
...
This will be used during deployment to apply migrations before any code changes are applied, ensuring no breakages in code due to a schema change.
...
Now that your local database is running, push your schema changes directly to the database without generating any migrations files using `drizzle-kit push` command:
...
Then add some content to the body of the request, stating the `text` key and value of the todo task.
...
![Add text content for the todo](/docs/images/guides/nitric-and-drizzle/step-3.png)

Let's toggle our todo as done, navigate to the `PATCH /todos/{id}/toggle` and enter the correct `id` to toggle.

![Add text content for the todo](/docs/images/guides/nitric-and-drizzle/step-4.png)

Finally, let's check our todo got toggled to done by calling the `GET /todos` endpoint.

![Add text content for the todo](/docs/images/guides/nitric-and-drizzle/step-5.png)
...
```yaml {{ title: 'nitric.staging-aws.yaml'}}
# The nitric provider to use
provider: nitric/aws@1.6.1
