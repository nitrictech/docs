export const description = 'Create and deploy schedules with Nitric'
...
<Note>
  Schedules aren't currently supported by Nitric on Azure due to the limitations
  of that provider.
</Note>
...
<div>
  <div className="video-container">
    <iframe
      src="https://www.youtube-nocookie.com/embed/mY6WazbNszU"
      title="Schedule Tasks from Your Cloud Application with Nitric"
      frameBorder="0"
...
Nitric.Schedule("aggregate-data").Every(3, Frequency.Days, ctx =>
{
  return ctx;
});
...
public class Application {
  public static void main(String[] args) {
    Nitric.INSTANCE.schedule("aggregate-data").every("3 days", (ctx) -> {
...
fun main() {
  Nitric.schedule("aggregate-data").every("3 days") { ctx ->
...
<Note>`cron` schedules aren't currently supported in the Python SDK</Note>
...
Nitric.Schedule("process-often").Every(5, Frequency.Minutes, ctx =>
{
  return ctx;
});

Nitric.Schedule("process-sometimes").Every(2, Frequency.Hours, ctx =>
{
  return ctx;
});

Nitric.Schedule("process-rarely").Every(30, Frequency.Days, ctx =>
{
  return ctx;
});
...
Nitric.INSTANCE.schedule("process-often").every("5 minutes", (ctx) -> {
  return ctx;
});

Nitric.INSTANCE.schedule("process-sometimes").every("2 hours", (ctx) -> {
  return ctx;
});

Nitric.INSTANCE.schedule("process-rarely").every("30 days", (ctx) -> {
  return ctx;
});
```

```kotlin
Nitric.schedule("process-often").every("5 minutes") { ctx ->
  ctx
}

Nitric.schedule("process-sometimes").every("2 hours") { ctx ->
  ctx
}

Nitric.schedule("process-rarely").every("30 days") { ctx ->
  ctx
}
```
...
```txt {{ language: 'python' }}
`cron` schedules aren't supported in the Python SDK currently
```
...
// every 15 minutes
nitric.NewSchedule("check for updates").Cron("0/15 * * * *", func (ctx *faas.EventContext, next faas.EventHandler) (*faas.EventContext, error) {
...
- [AWS configuration](/reference/providers/aws/configuration)
- [Google Cloud configuration](/reference/providers/gcp/configuration)
