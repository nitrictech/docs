...
<Note>
  Previous versions of Nitric relied on two terminal windows to run
  ...
  that the index file that was previously provided in the templates is no
  longer required.
</Note>
...
In v1.x security definitions, now called rules, are created as standalone objects and then applied to the API or route. This make the ensures unapplied security definitions are more obvious and less likely to be overlooked.
