...
func main() {
	publicApi, err := resources.NewApi("public")
	if err != nil {
		return
	}

  // ✅ This declaration will work
  files, err := resources.NewBucket("files").With(resources.BucketReading)
  if err != nil {
    return
  }
  ...
  profiles.Get("/file/:name", func(ctx *faas.HttpContext, next faas.HttpHandler) (*faas.HttpContext, error) {
    // ❌ This declaration will not work, as this is only called at runtime.
    badBucket, err := resources.NewBucket("wont-work").With(resources.BucketWriting)
...
func NewManager() (*Resources, error) {
	if Manager == nil {
		...
		bucketOne, err := resources.NewBucket("bucket-one").With(resources.BucketReading)
		if err != nil {
			return nil, err
		}

		bucketTwo, err := resources.NewBucket("bucket-two").With(resources.BucketReading)
		if err != nil {
			return nil, err
		}
...
import (
	...
	"github.com/nitrictech/go-sdk/faas"
	nitric "github.com/nitrictech/go-sdk/resources"

	resources "demoproject"
)
