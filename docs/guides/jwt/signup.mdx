...
This sample project will demonstrate signing up Users in a Nitric Document Collection.
...
- Set up a basic react app to consume the signup operation
- Set up an [API gateway](../api-gateways.mdx) and bind it to the functions
...
Let's pull it all together, modify or create the golang15 function -
...
func NitricFunction(trigger *faas.NitricTrigger) (*faas.NitricResponse, error) {
	...
	// Add the code for the folowing steps here 
...
Update the CSS file contents - index.css
...
Once again we can run our service and site 

```bash
nitric run
```

Take note of the entry point URL, by control clicking we will launch into our site.
```bash
✔ Building Services
✔ Creating docker network
