...
func userExists(email string) bool {
	...
	query := docs.Collection(COLLECTION_USER).Query().Where(
		documents.Condition("Email").Eq(documents.StringValue(email)),
	)
	results, err := query.Fetch()

	if err != nil || len(results.Documents) != 1 {
		return false
	}
	return true
}
...
body {
  ...
  --theme-box-shadow: 0px 2px 4px -1px rgb(0 0 0 / 20%), 0px 4px 5px 0px rgb(0 0
          0 / 14%), 0px 1px 10px 0px rgb(0 0 0 / 12%);
