...
// Struct which represents the outgoing data
type ReturnData struct {
	Product string
	Domain  DomainData
}

// Struct which represents the Domain data
...
If an error occured, then let's send back a bad request status to the CheckoutSessionLineItemParams
```golang15
