...
<Note>
  We add the extra dependencies to the 'ml' category to keep them separate since
  they can be quite large. This lets us just install them in the containers that
  need them.
</Note>
...
Since the template we started with already had a service defined, we'll remove the existing hello world service and create a new service for our API.

```bash
rm services/hello.py
touch services/api.py
```

Now we can create the API endpoint in `services/api.py`.
...
```dockerfile title: docker/standard.dockerfile
...
```gitignore title: docker/standard.dockerfile.dockerignore
.mypy_cache/
.nitric/
.venv/
.model/
...
Let's also update the `nitric.yaml` to specify the new dockerfiles.
