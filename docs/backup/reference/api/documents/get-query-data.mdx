...
To retrieve the contents of a document reference use its `get()` method. Many of the SDKs provide
...
<CodeSnippet snippet="nitrictech/go-sdk/examples/documents/query.go" />
...
In the examples below we are selecting `Customers` documents where the customer's country is equal to 'US' and
...
By default the `fetch()` method will return all the documents in a collection. When working with very large collections this may not be
what you want. With some databases such as DynamoDB they have hard limits and will return an error if more than 1 MB of data has been fetched.

To limit the number of documents retrieved use the `limit()` method.
...
To use a paginated query set the page size `limit()`, and then use the `pagingToken` returned with your results as input into your
next query with the `pagingFrom()` method. This will enable the next query to continue from where the previous query finished.

The example below selects active 'Customers' by paging through the collection in sets of 100 document.
...
The Query object also supports streaming query results. To return your documents one by one use the
Query object's `stream()` method. Streams can better support functional programming models and
reduce memory usage.
