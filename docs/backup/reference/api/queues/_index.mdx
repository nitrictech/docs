...
Nitric Queues are designed to facilitate **Pull** based workloads, where tasks are sent by producers and received by consumers. Both ends of the transaction are requests, i.e. the consumer asks the queue for more tasks, the queue doesn't push tasks to the consumers.
This flow is useful in situations such as batch processing or when consumers aren't always available.

The Nitric Queues API provides an interface to queue and queue-like products such as AWS SQS, Google PubSub _(Pull Subscriptions)_ and Azure Storage Queues to facilitate message queueing for decoupled and distributed systems.
...
- **Tasks** are a combination of task metadata, a payload and, when received, a lease. Typically, a task payload will take the form of a JSON document.
- **Producers** produce tasks by sending them to queues. Unlike Event Publishers, Producers may be aware of the intended consumer of a tasks, since a queue can be intended for a single Consumer.
