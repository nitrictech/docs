...
<img height="134" src="/img/queues-overview.svg" alt="queue overview diagram" />
...
await queueClient.send(
  'my-queue',
  new Task((payload = { example: 'payload' }))
);
```
...
# Publish a task to a queue
queue_client.send("my-queue", { payload: { example: "payload" } });
```
...
import (
	...
	"github.com/nitrictech/go-sdk/queueclient"
)
...
_, err := client.Queue().Send(&queueclient.SendOptions{
	Queue: "my-queue",
	Task: &queueClient.Task{
		ID: "1234",
		PayloadType: "test-payload",
...
$task->setId("1234");
$task->setPayloadType("example-payload");
$task->setPayload((object)array("example" => "payload"));

$queueClient->send("my-queue", $task);
```
...
var client = QueueClient.NewBuilder()
	.Queue("my-queue")
	.Build()
...
const tasks = await queueClient.receive('my-queue', 5);

tasks.forEach((task) => {
  // Work on a task...

  // Complete the task
  queueClient.complete('my-queue', task);
});
```
...
# Receive tasks from the queue
for task in queue_client.receive("my-queue", 5):
	// Work on a task...

	// Complete the task
	queue_client.complete("my-queue", task)
```
...
var client = QueueClient.build("example");
...
for (Task task: tasks) {
	try {
		...
		client.complete(task.getLeaseId());
	} catch (Exception e) {
...
// Receive tasks from the queue
response, err := client.Queue().Receive(&queueclient.ReceiveOptions{
	Queue: "my-queue",
	Depth: 1,
})
...
for _, task := range response.Tasks {
	...
	client.Queue().Complete(&queueclient.CompleteOptions{
		Queue: "my-queue",
		Task: task,
	})
}
...
$tasks = $queueClient.receive("my-queue")

foreach($tasks as $task) {
	// process the task here...
	$queueClient->complete("my-queue", $task->getLeaseId())
}
```
...
var tasks = client.Receive(1);

foreach (Task t in tasks) {
	// process tasks

	// Complete the task
	client.Complete(t.LeaseID);
}
```
