...
languages={[
	{
		label: "Good nitric.yaml 👍",
		value: "good"
	},
	{
		label: "Bad nitric.yaml 👎",
		value: "bad"
	},
  {
		label: "api.yaml",
  ...
  main:
    paths:
      /:
        target: app
        type: site
      # 👍 Matches the base path of the above API (see api.yaml)
      /exampleapi/:
        target: exampleapi
        type: api
```
  ...
  main:
    paths:
      /:
        target: app
        type: site
      # 👎 Doesn't match the base path of the above API (see api.yaml)
      /myapi/:
        target: exampleapi
        type: api
```
...
paths:
  # 👀 entrypoint path must match leading path of API '/exampleapi/'
  /exampleapi/example:
    get:
      operationId: example
      x-nitric-target:
