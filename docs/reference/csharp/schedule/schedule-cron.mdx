...
Application.Schedule("send-reminder").Cron("0 1 1 * *", ctx => {
    // do some processing
});
...
// every 15 minutes
Application.Schedule("check for updates").Cron("0/15 * * * *", ctx => {
    Console.WriteLine("checking for updates");
});

// at 1:00 am on the 1st of every month
Application.Schedules("delete stale data").Cron("0 1 1 * *", ctx => {
