...
Once you have tested your application locally and you're ready to deploy, you can do `nitric up`. This command can be optionally supplied with the name of a stack with the `-s` or `--stack` argument. However, the stack will be auto-detected if theres only one or options will be presented if theres multiple. This will set the cloud configuration for that particular deployment.
