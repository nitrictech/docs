...
During deployment Nitric's CLI builds your API's routes, methods and handlers will be built as follows:
  ...
  - Files containing handler functions, such as [api.post()](/docs/reference/apis/api-get), are built into container images
  ...
  - IAM permissions are created enabling API Gateway to execute the Lambdas