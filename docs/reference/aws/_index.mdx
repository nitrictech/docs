...
The AWS provider is supported by the Nitric SDKs and CLI by default. However, credentials for an AWS account will be required when using the [deploy command](/docs/reference/) from the CLI.
...
You can create an Access Key by logging into the <a href="https://aws.amazon.com/console/" target="_blank">AWS console</a> and navigating to:
  `Username Menu > My Security Credentials > Access keys > Create New Access Key`
