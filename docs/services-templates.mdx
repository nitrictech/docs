Nitric Templates provide standard scaffolding and build processes for [Services](./services.mdx) and [Functions](./services-functions.mdx). When you add a new Service/Function to your project you'll be asked which template to build it from, the template contents will be copied into the src directory for your new service to be used as a starting point for development.

Later, when using the `run`, `build` or `deploy` commands with the Nitric CLI, the template will be referenced again to determine how your Service/Function should be built into a container and configured to execute with the [Nitric Membrane](./nitric-membrane.mdx). For this reason, every service you add to a Nitric projects will maintain a reference to the template it's based on.
