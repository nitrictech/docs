The Nitric framework enables building a wide range of multi-cloud solutions. This topic discusses how to organize your projects with Nitric to support a common cloud architectures and development practices.

Before diving into projects lets first define some common terminology.

## Core Terminology

### Stack
A Stack is a collection of resources defined using a `nitric.yaml`. Stacks typically define some code functions and cloud resources. Some stacks are infrastructure only, deploying shared resources and act as a dependency to other stacks. A REST API would typically be defined in a Stack.

### Service
A Service is a cohesive set of functionality provided within the bounded-context of a *Stack*. A service may be implemented by one or more *Functions* / *Containers*, with their closely related cloud resources, such as buckets, queues, etc. An example Service would be a REST API or a Microservice in a larger enterprise architecture.

### Function
A Function a deployable unit of code to handle a single purpose.  Functions are analogous to an AWS Lambda functions or GCP Cloud Functions and provide the code building blocks for creating larger *Services* and *Applications*.

Nitric Functions are packaged an deployed as separate OCI containers.

An example Function could be a Get Customer Record method in a Customer REST API.

### Container
A Container in Nitric is a deployable OCI container or Docker image. Containers are used to package and deploy Nitric Functions, but they can also be used to deploy full stack applications or microservices using frameworks like ExpressJS or SpringBoot.

### Application
An Application is one or more Stacks which together provide all required functionality for a solution.


## Project Types

With Nitric there 3 primary types of projects:

* Single Stack projects, which contains a single Stack definition
* Resource Project, which contains a single Stack but no Functions
* Multi Stack projects, which contain multiple Stack definitions

### Single Stack

A Single Stack example is provided below:

<CodeExamples 
	languages={[
	{
		label: "Node",
		value: "node"
	}, {
		label: "Python",
		value: "python"
	}, {
		label: "Java",
		value: "java"
	}, {
		label: "Go",
		value: "go"
	}
	]}
	defaultLang="typescript"
>
<CodeExample lang={"node"}>

```bash
+--commmon/
|  +--*.ts
|
+--functions/
|  +--create.ts
|  +--list.ts
|  +--read.ts
|
+--api.yaml
+--nitric.yaml
+--package.json
```

</CodeExample>
<CodeExample lang={"python"}>

```bash
+--commmon/
|  +--__init__.py
|  +--*.py
|
+--functions/
|  +--__init__.py
|  +--create.py
|  +--list.py
|  +--read.py
|
+--api.yaml
+--nitric.yaml
+--requirements.txt
```

</CodeExample>
<CodeExample lang={"java"}>

```bash
+--commmon/
|  +--src/
|  +--pom.xml
|
+--functions/
|  |
|  +--create/
|  |  +--src/
|  |  +--pom.xml
|  |
|  +--list/
|  |  +--src/
|  |  +--pom.xml
|  |
|  +--read/
|     +--src/
|     +--pom.xml
|
+--api.yaml
+--nitric.yaml
+--pom.xml
```

</CodeExample>
<CodeExample lang={"go"}>

```bash
+--commmon/
|  +--*.go
|
+--functions/
|  |
|  +--create/
|  |  +--main.go
|  |
|  +--list/
|  |  +--main.go
|  |
|  +--read/
|     +--main.go
|
+--api.yaml
+--nitric.yaml
+--go.mod
```

</CodeExample>
</CodeExamples> 

### Resource Project

Resource projects can be used for deployable websites which don't contain any functions. Resource projects can also be used to define cloud resources that are shared across multiple projects.

A website Resource Project example is provided below:

```
+--website/
|
+--nitric.yaml
```

### Multi Stack

A Multi Stack example is provided below:

<CodeExamples 
	languages={[
	{
		label: "Node",
		value: "node"
	}, {
		label: "Python",
		value: "python"
	}, {
		label: "Java",
		value: "java"
	}, {
		label: "Go",
		value: "go"
	}
	]}
	defaultLang="typescript"
>
<CodeExample lang={"node"}>

```bash
+--common/
|  +--*.ts
|
+--services/
|  +--customer/
|  |  +--functions/
|  |  |  +--create.ts
|  |  |  +--list.ts
|  |  |  +--read.ts
|  |  |
|  |  +--api.yaml
|  |  +--nitric.yaml
|  |  +--package.json
|  |
|  +--order/
|     +--functions/
|     |  +--open.ts
|     |  +--search.ts
|     |  +--update.ts
|     |
|     +--api.yaml
|     +--nitric.yaml
|     +--package.json
|       
+--package.json 
```

</CodeExample>
<CodeExample lang={"python"}>

```bash
+--commmon/
|  +--__init__.py
|  +--*.py
|
+--services/
|  +--customer/
|  |  +--functions/
|  |  |  +--__init__.py
|  |  |  +--create.py
|  |  |  +--list.py
|  |  |  +--read.py
|  |  |
|  |  +--nitric.yaml
|  |
|  +--order/
|     +--functions/
|     |  +--__init__.py
|     |  +--open.py
|     |  +--search.py
|     |  +--update.py
|     |
|     +--nitric.yaml
|
+--requirements.txt
```

</CodeExample>
<CodeExample lang={"java"}>

```bash
+--commmon/
|  +--src/
|  +--pom.xml
|
+--services/
|  +--customer/
|  |  +--functions/
|  |  |  +--create/
|  |  |  |  +--src/
|  |  |  |  +--pom.xml
|  |  |  |
|  |  |  +--list/
|  |  |  |  +--src/
|  |  |  |  +--pom.xml
|  |  |  |
|  |  |  +--read/
|  |  |     +--src/
|  |  |     +--pom.xml
|  |  |
|  |  +--api.yaml
|  |  +--nitric.yaml
|  |
|  +--order/
|     +--functions/
|     |  +--open/
|     |  |  +--src/
|     |  |  +--pom.xml
|     |  |
|     |  +--search/
|     |  |  +--src/
|     |  |  +--pom.xml
|     |  |
|     |  +--update/
|     |     +--src/
|     |     +--pom.xml
|     |
|     +--api.yaml
|     +--nitric.yaml
|       
+--pom.xml
```

</CodeExample>
<CodeExample lang={"go"}>

```bash
+--commmon/
|  +--*.go
|
+--services/
|  +--customer/ 
|  |  +--functions/
|  |  |  +--create/
|  |  |  |  +--main.go
|  |  |  |
|  |  |  +--list/
|  |  |  |  +--main.go
|  |  |  |
|  |  |  +--read/
|  |  |     +--main.go
|  |  |
|  |  +--api.yaml
|  |  +--nitric.yaml
|  |  +--go.mod
|  |
|  +--order/ 
|     +--functions/
|     |  +--open/
|     |  |  +--main.go
|     |  |
|     |  +--search/
|     |  |  +--main.go
|     |  |
|     |  +--update/
|     |     +--main.go
|     |
|     +--api.yaml
|     +--nitric.yaml
|     +--go.mod
|
+--go.mod
```

</CodeExample>
</CodeExamples> 


# Project Solutions

Nitric Single or Multi Stack projects can be applied to a variety of solution architectures, development practices and team organizations.

## Small Solutions

Small solutions are generally developed by a single team. Examples of small solution projects include a mobile application backend or a standalone service.

For these types of projects a Single Stack is a good choice as it:

* supports a cohesive set of functionality which is deployed together
* enables easy code sharing across project functions
* provides good team productivity, its easy to manage and easy to get started with

#### Project Example

A Small Solution project example simply uses a Single Stack project repository.

<CodeExamples 
	languages={[
	{
		label: "Node",
		value: "node"
	}, {
		label: "Python",
		value: "python"
	}, {
		label: "Java",
		value: "java"
	}, {
		label: "Go",
		value: "go"
	}
	]}
	defaultLang="typescript"
>
<CodeExample lang={"node"}>

```bash
+--commmon/
|  +--*.ts
|
+--functions/
|  +--create.ts
|  +--list.ts
|  +--read.ts
|
+--website/
|
+--api.yaml
+--nitric.yaml
+--package.json
```

</CodeExample>
<CodeExample lang={"python"}>

```bash
+--commmon/
|  +--__init__.py
|  +--*.py
|
+--functions/
|  +--__init__.py
|  +--create.py
|  +--list.py
|  +--read.py
|
+--website/
|
+--api.yaml
+--nitric.yaml
+--requirements.txt
```

</CodeExample>
<CodeExample lang={"java"}>

```bash
+--commmon/
|  +--src/
|  +--pom.xml
|
+--functions/
|  |
|  +--create/
|  |  +--src/
|  |  +--pom.xml
|  |
|  +--list/
|  |  +--src/
|  |  +--pom.xml
|  |
|  +--read/
|     +--src/
|     +--pom.xml
|
+--website/
|
+--api.yaml
+--nitric.yaml
+--pom.xml
```

</CodeExample>
<CodeExample lang={"go"}>

```bash
+--commmon/
|  +--*.go
|
+--functions/
|  |
|  +--create/
|  |  +--main.go
|  |
|  +--list/
|  |  +--main.go
|  |
|  +--read/
|     +--main.go
|
+--website/
|
+--api.yaml
+--nitric.yaml
+--go.mod
```

</CodeExample>
</CodeExamples> 


## Large Solutions

Large Solutions are typically composed of multiple services, with multiple development teams and often using different languages. For these types of solutions multiple Nitric Stacks are require to support the different services.

There are two recommended approaches for tackling these larger solutions.

* **Multi-Repo**
* **Mono Repo** 

### Multi-Repo

The Multi-Repo strategy uses a separate project repository for each Service with its own Stack (Single Stack projects).

The Multi-Repo strategy is a good choice for:

* decoupled services
* multiple development teams
* supporting different service development programming languages

With the Multi-Repo strategy code sharing within a service is important. However, sharing code across services is intentionally eliminated to reduce coupling.

When common utility libraries need to be shared across services, this should be done through traditional public/private package manages such as NPM or Maven. These common libraries should follow rigours semantic version practices to ensure services can be developed and deployed independently.

Also note both Simple Solutions and Large Multi-Repos Solution use Single Stack projects, making it is easy for developers to migrate between these projects.

While Multi-Repo provides a highly scalable strategy for building large solutions, it does require additional effort to managing development and deployment of services across teams. 

#### Multi-Repo Example

Examples of a Multi-Repo strategy is provided below with separate repositories for:
* Customer Service
* Order Service
* Shared Library
* Website Resources

##### Customer Service

A Customer Service developed in a Single Stack project repository.

<CodeExamples 
	languages={[
	{
		label: "Node",
		value: "node"
	}, {
		label: "Python",
		value: "python"
	}, {
		label: "Java",
		value: "java"
	}, {
		label: "Go",
		value: "go"
	}
	]}
	defaultLang="typescript"
>
<CodeExample lang={"node"}>

```bash
+--commmon/
|  +--*.ts
|
+--functions/
|  +--create.ts
|  +--list.ts
|  +--read.ts
|
+--api.yaml
+--nitric.yaml
+--package.json
```

</CodeExample>
<CodeExample lang={"python"}>

```bash
+--commmon/
|  +--__init__.py
|  +--*.py
|
+--functions/
|  +--__init__.py
|  +--create.py
|  +--list.py
|  +--read.py
|
+--api.yaml
+--nitric.yaml
+--requirements.txt
```

</CodeExample>
<CodeExample lang={"java"}>

```bash
+--commmon/
|  +--src/
|  +--pom.xml
|
+--functions/
|  |
|  +--create/
|  |  +--src/
|  |  +--pom.xml
|  |
|  +--list/
|  |  +--src/
|  |  +--pom.xml
|  |
|  +--read/
|     +--src/
|     +--pom.xml
|
+--api.yaml
+--nitric.yaml
+--pom.xml
```

</CodeExample>
<CodeExample lang={"go"}>

```bash
+--commmon/
|  +--*.go
|
+--functions/
|  |
|  +--create/
|  |  +--main.go
|  |
|  +--list/
|  |  +--main.go
|  |
|  +--read/
|     +--main.go
|
+--api.yaml
+--nitric.yaml
+--go.mod
```

</CodeExample>
</CodeExamples> 

##### Order Service

An Order Service is developed is a separate Single Stack project repository.

<CodeExamples 
	languages={[
	{
		label: "Node",
		value: "node"
	}, {
		label: "Python",
		value: "python"
	}, {
		label: "Java",
		value: "java"
	}, {
		label: "Go",
		value: "go"
	}
	]}
	defaultLang="typescript"
>
<CodeExample lang={"node"}>

```bash
+--commmon/
|  +--*.ts
|
+--functions/
|  +--open.ts
|  +--search.ts
|  +--update.ts
|
+--api.yaml
+--nitric.yaml
+--package.json
```

</CodeExample>
<CodeExample lang={"python"}>

```bash
+--commmon/
|  +--__init__.py
|  +--*.py
|
+--functions/
|  +--__init__.py
|  +--open.py
|  +--search.py
|  +--update.py
|
+--api.yaml
+--nitric.yaml
+--requirements.txt
```

</CodeExample>
<CodeExample lang={"java"}>

```bash
+--commmon/
|  +--src/
|  +--pom.xml
|
+--functions/
|  |
|  +--open/
|  |  +--src/
|  |  +--pom.xml
|  |
|  +--search/
|  |  +--src/
|  |  +--pom.xml
|  |
|  +--update/
|     +--src/
|     +--pom.xml
|
+--api.yaml
+--nitric.yaml
+--pom.xml
```

</CodeExample>
<CodeExample lang={"go"}>

```bash
+--commmon/
|  +--*.go
|
+--functions/
|  |
|  +--open/
|  |  +--main.go
|  |
|  +--search/
|  |  +--main.go
|  |
|  +--update/
|     +--main.go
|
+--api.yaml
+--nitric.yaml
+--go.mod
```

</CodeExample>
</CodeExamples> 

##### Shared Library

Utility libraries shared across services are developed in separate repositories using standard language package and dependency management features.

<CodeExamples 
	languages={[
	{
		label: "Node",
		value: "node"
	}, {
		label: "Python",
		value: "python"
	}, {
		label: "Java",
		value: "java"
	}, {
		label: "Go",
		value: "go"
	}
	]}
	defaultLang="typescript"
>
<CodeExample lang={"node"}>

```bash
+--src/
|  +--*.ts
|
+--package.json
```

</CodeExample>
<CodeExample lang={"python"}>

```bash
+--src/
|  +--*.py
|
+--requirements.txt
```

</CodeExample>
<CodeExample lang={"java"}>

```bash
+--src/
|
+--pom.xml
```

</CodeExample>
<CodeExample lang={"go"}>

```bash
+--src/
|
+--go.mod
```

</CodeExample>
</CodeExamples> 

##### Website Resources

If the solution includes a front-end then you will typically have a frontend resources project for deploying the static website content.

```
+--website/
|
+--nitric.yaml
```

### Mono-Repo

The Mono-Repo strategy uses a single source code repository for all Services. While each Service still has its own Stack, the repository is a Multi Stack project.

The Mono-Repo strategy is a good choice for:

* larger applications composed of many services
* services may have some coupling, and which should be deployed together
* teams collaborating closely together to maximise productivity 

Mono-Repo projects can be managed as rigorously as Multi-Repo projects, but they also provide the option to dial back some of separation. 

If your solution should be version together, then a Mono-Repo is probably a good fit.

#### Project Example

A Mono-Repo example of a is provided below using a Multi-Stack project.  This project includes:

* Customer Service with its own Stack
* Order Service with its own Stack
* Shared Library for common code
* Website Resources

<CodeExamples 
	languages={[
	{
		label: "Node",
		value: "node"
	}, {
		label: "Python",
		value: "python"
	}, {
		label: "Java",
		value: "java"
	}, {
		label: "Go",
		value: "go"
	}
	]}
	defaultLang="typescript"
>
<CodeExample lang={"node"}>

```bash
+--common/
|  +--*.ts
|
+--services/
|  +--customer/
|  |  +--functions/
|  |  |  +--create.ts
|  |  |  +--list.ts
|  |  |  +--read.ts
|  |  |
|  |  +--api.yaml
|  |  +--nitric.yaml
|  |  +--package.json
|  |
|  +--order/
|     +--functions/
|     |  +--open.ts
|     |  +--search.ts
|     |  +--update.ts
|     |
|     +--api.yaml
|     +--nitric.yaml
|     +--package.json
|
+--website/
|
+--nitric.yml
+--package.json 
```

</CodeExample>
<CodeExample lang={"python"}>

```bash
+--commmon/
|  +--__init__.py
|  +--*.py
|
+--services/
|  +--customer/
|  |  +--functions/
|  |  |  +--__init__.py
|  |  |  +--create.py
|  |  |  +--list.py
|  |  |  +--read.py
|  |  |
|  |  +--nitric.yaml
|  |
|  +--order/
|     +--functions/
|     |  +--__init__.py
|     |  +--open.py
|     |  +--search.py
|     |  +--update.py
|     |
|     +--nitric.yaml
|
+--website/
|
+--nitric.yml
+--requirements.txt
```

</CodeExample>
<CodeExample lang={"java"}>

```bash
+--commmon/
|  +--src/
|  +--pom.xml
|
+--services/
|  +--customer/
|  |  +--functions/
|  |  |  +--create/
|  |  |  |  +--src/
|  |  |  |  +--pom.xml
|  |  |  |
|  |  |  +--list/
|  |  |  |  +--src/
|  |  |  |  +--pom.xml
|  |  |  |
|  |  |  +--read/
|  |  |     +--src/
|  |  |     +--pom.xml
|  |  |
|  |  +--api.yaml
|  |  +--nitric.yaml
|  |
|  +--order/
|     +--functions/
|     |  +--open/
|     |  |  +--src/
|     |  |  +--pom.xml
|     |  |
|     |  +--search/
|     |  |  +--src/
|     |  |  +--pom.xml
|     |  |
|     |  +--update/
|     |     +--src/
|     |     +--pom.xml
|     |
|     +--api.yaml
|     +--nitric.yaml
|
+--website/
|
+--nitric.yml
+--pom.xml
```

</CodeExample>
<CodeExample lang={"go"}>

```bash
+--commmon/
|  +--*.go
|
+--services/
|  +--customer/ 
|  |  +--functions/
|  |  |  +--create/
|  |  |  |  +--main.go
|  |  |  |
|  |  |  +--list/
|  |  |  |  +--main.go
|  |  |  |
|  |  |  +--read/
|  |  |     +--main.go
|  |  |
|  |  +--api.yaml
|  |  +--nitric.yaml
|  |  +--go.mod
|  |
|  +--order/ 
|     +--functions/
|     |  +--open/
|     |  |  +--main.go
|     |  |
|     |  +--search/
|     |  |  +--main.go
|     |  |
|     |  +--update/
|     |     +--main.go
|     |
|     +--api.yaml
|     +--nitric.yaml
|     +--go.mod
|
+--website/
|
+--nitric.yml
+--go.mod
```

</CodeExample>
</CodeExamples> 