...
> ⚠️ WARNING: The Key Value store will be superseded in the near future by the Document Service. The Key Value service will remain available but Deprecated until the Nitric Membrane 1.0 release
...
Collections are a resource defined in a Nitric stack definition (`nitric.yaml`). At the root level of the stack file, add a `collections` key, which contains an array of collections.
