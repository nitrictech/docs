...
- `api.yaml` defines the [API](/docs/intro/concepts/nitric-apis).
- Three [Functions](/docs/intro/concepts/functions) are generated:
  - `functions/create/**/CreateFunction.java` is the function that creates new documents.
  - `functions/list/**/ListFunction.java` is the function that lists multiple documents.
  - `functions/read/**/ReadFunction.java` is the function that can read a single document.
...
- Takes an example request.
- Generates a unique id for the new document.
- Creates the new example document in the `example` collection.
