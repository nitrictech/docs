...
3. Then replace the mainClass contents `com.example.service.ReadHandler` with `com.example.service.DeleteHandler`.

```xml
<mainClass>com.example.service.DeleteHandler</mainClass>
```
...
5. In the `src/main/java/com/example/service` directory, rename the `ReadHandler.java` file to `DeleteHandler.java` and replace its contains with your new function code:
...
public class DeleteHandler implements HttpHandler {
    ...
    public DeleteHandler(Documents documents) {
    ...
    public static void main(String[] args) {
        ...
        var deleteHandler = new DeleteHandler(new Documents());
