...
You can install Nitric using the [Chocolatey package manager](https://chocolatey.org/):

```bash
choco install nitric
```

Future updates can be install using the `upgrade` command:

```bash
choco upgrade nitric
```
...
Alternatively, you download and execute our install script:
