...
At this point you have everything setup and ready to build applications, then run and test them locally. You can read more about the [Nitric CLI](./nitric-cli), try out a [Tutorial](./quick-start) or if you're ready to deploy an application to a cloud environment, you can install the relevant [CLI Plugin](./cli#plugins) for [AWS](./provider-aws), [Google Cloud](./provider-gcp), [DigitalOcean](./provider-digitalocean) or [Microsoft Azure](./provider-azure).