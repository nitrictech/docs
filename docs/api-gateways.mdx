Nitric's API Gateway resource provides a convenient option for configuring and deploying RESTful APIs in front of your application's [Functions](./services-functions) and other [Service](./services). These APIs are then deployed to the managed API Gateway services of each supported cloud provider to ensure their reliability, scalability and security.
