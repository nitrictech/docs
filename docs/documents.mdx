...
In the first example we are performing a collection subcollection query where we are fetching all "Orders" documents for all "Customers".
...
<br />
Ability to query a collection's subcollection is in development.
...
In the second example we are performing a collection document subcollection query where we are fetching all the "Orders" for a specific "Customer" with the id of 'apple'.
