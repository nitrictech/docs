...
var query = docs.Collection("Customers").Query();
...
Subcollection queries can be created either:
1. under a collection's subcollection `collection.collection.query`, or
2. under a collection document's subcollection `collection.document.collection.query`

In the first example we are performing a collection subcollection query where we 
are fetching all "Orders" documents for all "Customers".
...
const query = docs.collection('Customers')
		.collection('Orders')
		.query();
...
var query = docs.Collection("Customers")
		.Collection("Orders")
		.Query();
...
In the second example we are performing a collection document subcollection query where we 
are fetching all the "Orders" for a specific "Customer" with the id of 'apple'.
...
const stream = docs.collection('Customers')
		.query()
		.stream();
...
<br/> 
The steam() method is not yet implemented. Please use the QueryResults foreach() method instead.
To scroll over all collection results use the fetchAll() method.
...
<br/> 
The steam() method is not yet implemented.
